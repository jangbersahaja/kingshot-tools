...
export default function TroopsInput({
  ...
  const getCount = (type: TroopType, tier: TroopTier): number =>
    config.inventory.items.find((i) => i.type === type && i.tier === tier)
      ?.count ?? 0;
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs border-separate border-spacing-y-0.5">
          <thead>
            <tr>
              {/* Tier column header */}
              <th className="w-8 text-left pb-2 text-gray-500 font-medium">
                Tier
              </th>
              {TROOP_TYPES.map((type) => {
                ...
                return (
                  <th key={type} className="pb-2 px-1">
                    <div
                      className={`flex flex-col items-center gap-0.5 ${color}`}
                    >
            ...
            {TROOP_TIERS.map((tier) => {
              ...
              return (
                <tr
                  key={tier}
                  className={
                    hasAny ? "bg-white/4" : "opacity-60 hover:opacity-100"
                  }
                >
            ...
            <tr className="border-t border-white/10">
              ...
              {TROOP_TYPES.map((type) => {
                ...
                return (
                  <td
                    key={type}
                    className={`pt-2 px-1 text-right text-[10px] font-bold ${total > 0 ? color : "text-gray-600"}`}
                  >
